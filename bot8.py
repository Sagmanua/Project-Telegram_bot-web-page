...
def process_tank_info(message):
    ...
    # The new formatted response for a single tank
    response = (
...
def process_tank_compare(message):
    ...
    # Send the message, with a fallback just in case markdown formatting fails
    try:
...
def process_player_stats(message):
    ...
    try:
        ...
        # Add a button to check progress directly from here
        markup = InlineKeyboardMarkup()
...
def handle_tank_visuals(call):
    # Data format: "visual_type_tankname"
    parts = call.data.split('_')
    visual_type = parts[1]  # 'equipment' or 'crew'
    tank_name = parts[2]    # The name of the tank
    ...
    # Use your existing image processing function
...
def start(message):
    ...
    # Create the beautiful button menu
    ...
    markup.add(btn_tank, btn_compare, btn_player, btn_clan, btn_calc)

    welcome_text = (
        "👋 *Welcome to Tank Assistant!*\n\n"
        "I can help you find tank stats, check player progress, and manage your clan.\n"
...
def handle_main_menu(call):
    ...
    if call.data == "menu_tank":
        ...
        start(call.message) # Re-trigger the start menu
...
def handle_clan_broadcast_btn(call):
    # We explain to the user they need to reply to the message they want to send
...
def process_clan_message_logic(message):
    ...
    # Check if the user is replying to something specific (images, stickers, etc.)
    if message.reply_to_message:
        target_message = message.reply_to_message
    else:
        # Otherwise, just use the message they just typed
    ...
    for member_id in members:
        if member_id != sender_id:
            try:
                # copy_message works for text, photos, and video
                bot.copy_message(
                    chat_id=member_id,
