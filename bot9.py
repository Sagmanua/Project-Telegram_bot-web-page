...
def find_tank_data(category: str, tank_name: str):
    """Helper function to find a tank using fuzzy matching in combined_data."""
...
def start(message):
    ...
    markup.add(btn_tank, btn_compare, btn_player, btn_clan, btn_calc, btn_moe)
...
def handle_main_menu(call):
    ...
    if call.data == "menu_tank":
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🏆 Mastery", callback_data="moe_cat_mastery"))
        markup.add(InlineKeyboardButton("🎯 MoE (Gunmarks)", callback_data="moe_cat_gunmarks"))
        markup.add(InlineKeyboardButton("⬅️ Back to Main Menu", callback_data="menu_back"))
        bot.edit_message_text("🎖️ *MoE / Mastery*\nWhat data are you looking for?", chat_id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
...
def handle_mastery(message):
    ...
    # Simply reuse the next step handler's logic
